...
function windowLoadInit() {
	...
	jQuery('form.contact-forms').on('submit', function( e ){
		e.preventDefault();
		var $form = jQuery(this);
		jQuery($form).find('span.contact-forms-respond').remove();
		...
		var ajax = jQuery.post( "contact-forms.php", request )
		.done(function( data ) {
			jQuery($form).find('[type="submit"]').attr('disabled', false).parent().append('<span class="contact-forms-respond highlight">'+data+'</span>');
			//cleaning form
		...
		.fail(function( data ) {
			jQuery($form).find('[type="submit"]').attr('disabled', false).parent().append('<span class="contact-forms-respond highlight">Mail cannot be sent. You need PHP server to send mail.</span>');
		})
	});
	...
	jQuery('.testimonials-owl-content').each(function() {
		...
		var $owl1 = $owl2.next('.testimonials-owl-dots');
